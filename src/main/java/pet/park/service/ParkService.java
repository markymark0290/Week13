...
//import pet.park.controller.List;
...
//		@formatter:off
		return contributorDao.findAll()
				.stream()
				.map(ContributorData::new)   //.map(cont -> new ContributorData(cont))
				.toList();
//		@formatter:on
	}
	
	@Transactional(readOnly = true)
	public ContributorData retrieveContributorsById(Long contributorId) {
		Contributor contributor = findContributorById(contributorId);
		return new ContributorData(contributor);
	}

	@Transactional(readOnly = false)
	public void deleteContributorID(Long contributorId) {
		Contributor contributor = findContributorById(contributorId);
	...
	public PetParkData savePetPark(Long contributorId, PetParkData petParkData) {
		...
		return new PetParkData(dbPetPark); //what does this do?
	}
	...
	private PetPark findPetParkById(Long petParkId) {
		...
		return petParkDao.findById(petParkId).orElseThrow(
				() -> new NoSuchElementException("Pet park with Id=" + petParkId + " does not exist.")); 
	}
